...
// Shared core logic for processing due booking reminders (email + SMS). Called from
// two places: the existing HTTP route (src/routes/api/public/hooks/send-booking-reminders.ts,
// kept for manual/debug triggering) and the new Cloudflare Cron Trigger task
// (tasks/send-booking-reminders.ts) that replaces Lovable Cloud's "Jobs" schedule
// (previously ran this same logic every 5 minutes via Lovable's own scheduler).
...
export interface BookingReminderRunResult {
  processed: number
  sent: number
  failed: number
  skipped: number
...
export async function processBookingReminders(supabase: any): Promise<BookingReminderRunResult> {
